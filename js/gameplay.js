...
function animate3D() {
    requestAnimationFrame(animate3D); handlePlayerMovement();
    ...
    if (!isPaused && timeLeft > 0 && hp > 0 && loadingScreen.style.display === 'none' && instructionsOverlay.style.display !== 'flex') {
        ...
        for (let k = powerups3D.length - 1; k >= 0; k--) {
            let pup = powerups3D[k]; pup.mesh.rotation.y += 0.04;
        ...
        for (let i = pigs3D.length - 1; i >= 0; i--) {
            ...
            obstacles.forEach(obs => {
                ...
                if(distToObs < obs.radius + 1.2) { let pushDir = new THREE.Vector3().subVectors(pig.mesh.position, new THREE.Vector3(obs.x, 0, obs.z)).normalize(); avoidance.addScaledVector(new THREE.Vector3(-pushDir.z, 0, pushDir.x), pig.speed * 1.5); }
            });
            pigPens.forEach(pen => {
                let distToPen = new THREE.Vector3(pig.mesh.position.x, 0, pig.mesh.position.z).distanceTo(new THREE.Vector3(pen.x, 0, pen.z));
                if (distToPen < pen.radius + 0.5) { let dx = pig.mesh.position.x - pen.x; let dz = pig.mesh.position.z - pen.z; if (!(dz > 1.5 && Math.abs(dx) < 1.0)) { let pushPen = new THREE.Vector3(dx, 0, dz).normalize(); avoidance.addScaledVector(pushPen, pig.speed * 2.0); } }
            });

            let finalMove = dirToTarget.clone().multiplyScalar(pig.speed).add(avoidance); pig.mesh.position.add(finalMove);
...
function handlePlayerMovement() {
    ...
    let dynamicSpeed = playerBaseSpeed * currentSpeedMultiplier;
    ...
    if (isJumping) { camera.position.y += jumpVelocity; jumpVelocity -= gravity; let checkGround = playerCurrentGroundY + playerHeightOffset; if (camera.position.y <= checkGround) { camera.position.y = checkGround; isJumping = false; jumpVelocity = 0; } } 
    else { let targetEyeHeight = playerCurrentGroundY + playerHeightOffset; if(camera.position.y > targetEyeHeight) { camera.position.y -= 0.2; if(camera.position.y < targetEyeHeight) camera.position.y = targetEyeHeight; } else { camera.position.y = targetEyeHeight; } }
